...
Slack を使う設定では、Shepherd は `allowed_users` を必須にします。daemon は `allowed_teams`、`allowed_channels`、`allowed_users` から外れた message を無視し、拒否した軸と Slack ID を debug level で記録します。`tui_default_channel` を設定する場合は、同じ channel ID を `allowed_channels` に含めてください。
...
daemon を起動する shell で token を入力します。この方法なら token 値が shell history に残りません。
...
Pi runtime を有効にして daemon を起動する前に、ローカルの Shepherd Pi package をインストールします:
...
Pi に利用できる model がない場合は、Pi を一度開いて `/login` を実行します。`shepherd daemon` は `pi --mode rpc --no-session` を起動し、`shepherd-pi` extension の handshake を待ち、Pi に authenticated model が 1 つ以上あることを確認します。
...
running daemon session に message を送ります:
...
- `src/daemon`: JSON Lines framing と recovery を含む local daemon utility。
...
- `src/gateway`: provider adapter、logical tool、turn queueing、context、summary update。
...
- `src/tui`: TUI-style local surface が使う daemon socket client。
