...
- `shepherd send --session <id> --text <text> --provider <name> --model <id>`: その message の gateway provider/model を override する。
...
- `shepherd-tools --socket <path>`: daemon の `tool.list` / `tool.run` を stdio JSON Lines で bridge する。
...
- `docs/plans/archived/`: 完了済み plan の退避先。
...
- `src/gateway/working-contexts.ts`: allowed-root working context discovery / resolution。
...
- Active plan は `docs/plans/` 配下に置く。命名は `YYYY-MM-DD-short-slug.md` を基本にする。
- 親 plan に紐づく子 plan は `docs/plans/<parent-slug>/YYYY-MM-DD-topic-slug.md` に置く。
- plan が実装・検証・docs 更新まで完了したら、関連する plan 群を `docs/plans/archived/` に移動する。
- 並行セッションで plan 更新が入っている、または追加判断が残っている場合は archive しない。
- plan を移動するときは、親子リンク、README/AGENTS などの参照、関連する相対リンクを同じ commit で更新する。
- archive は実装変更とは分けて、docs-only commit にする。
