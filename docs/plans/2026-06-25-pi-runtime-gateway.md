...
- **In progress** — Implementation slice 1: config schema accepts `gateway.pi`, daemon startup checks Pi readiness, and daemon RPC has the external run queue lifecycle for fake Pi extension claim/complete/fail.
